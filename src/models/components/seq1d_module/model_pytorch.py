"""Based on scripts from Tranception

TODO:
1. change full seq autoregressive to ABI
...
class TranceptionModel(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        if input_ids is not None and inputs_embeds is not None:
            ...
            input_shape = input_ids.size() # input_ids:[b,l]?
            input_ids = input_ids.view(-1, input_shape[-1])
            batch_size = input_ids.shape[0]
        elif inputs_embeds is not None:
            input_shape = inputs_embeds.size()[:-1] # inputs_embeds:[b,l,d]?
        ...
        if token_type_ids is not None:
            token_type_ids = token_type_ids.view(-1, input_shape[-1]) # [b,l]
        if position_ids is not None:
            position_ids = position_ids.view(-1, input_shape[-1]) # [b,l]

        if past_key_values is None:
            past_length = 0
            past_key_values = tuple([None] * len(self.h)) # [num_hidden_layers,]
        else:
            past_length = past_key_values[0][0].size(-2) # past_key_values:[num_h_l,num_heads,l,~,~]?
...
class TranceptionLMHeadModel(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            if self.config.task_mode == "AR":
                # Shift so that tokens < n predict n
                shift_logits = lm_logits[..., :-1, :].contiguous()
                shift_labels = labels[..., 1:].contiguous()
            elif self.config.task_mode == 'ABI':
