...
from model.tokenizers import BaseTokenizer
...
class PdbDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # PLM aa-seq inputs
        aa_ids = chain_feats['aatype'].numpy() # long tensor (converted to ids)
        ...
        masked_aa_tokens, labels = self._apply_bert_mask(aa_tokens)
        masked_aa_ids = np.array(self.tokenizer.convert_tokens_to_ids(masked_aa_tokens), np.int32)
        input_aa_mask = np.ones_like(masked_aa_ids)
        chain_feats['input_aa_ids'] = masked_aa_ids
        chain_feats['input_aa_mask'] = input_aa_mask
        chain_feats['target_aa_ids'] = labels

        # Convert all features to tensors.
        final_feats = tree.map_structure(
            lambda x: x if torch.is_tensor(x) else torch.tensor(x), chain_feats)
        final_feats = du.pad_feats(final_feats, csv_row['modeled_seq_len']) #TODO: change padding
        if self.is_training:
            return final_feats
        else:
